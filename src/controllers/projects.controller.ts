...
export const getProjectsList = async (req: FastifyRequest, reply: FastifyReply) => {
  try {
    const result = await projectsModel.getAll(req.lang);
...
export const getProjectsById = async (
  ...
  try {
    const result = await projectsModel.getById(req.params.id, req.lang);
...
export const createProjects = async (
  ...
  try {
    const result = await projectsModel.insert(req.body, req.lang);
    const item = result.rows[0];
    item.cover_image = item.cover_image;
...
export const updateProjects = async (
  req: FastifyRequest<{ Params: { id: string }; Body: UpdateProjectsType }>,
  reply: FastifyReply
) => {
  try {
    const result = await projectsModel.update(req.params.id, req.body, req.lang);
    if (result.rows.length === 0) {
      return reply.status(404).send({ message: "Not found" });
    }
    const item = result.rows[0];
...
export const deleteProjects = async (
  ...
  try {
    const result = await projectsModel.delete(req.params.id, req.lang);
    if (result.rows.length === 0) {
      return reply.status(404).send({ message: "Not found" });
    }
